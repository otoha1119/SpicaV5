...
全20症例が先頭スライスから 1 対 1 で対応する。
ただし末尾の枚数が 3 症例で異なるため `min(n1024, n512)` までを使う:
`PCD-006` 1251/1290、`PCD-011` 1058/1075、`PCD-015` 1290/1264。

検証は `verify_pcd1024_pairing.py`。Z 間隔が 0.1mm で隣接スライスがほぼ同一なため
「同一インデックスで NCC が高い」だけでは不十分（±5 枚ずれても 0.99 が出る）。
周辺 ±20 スライスを総当たりして **同一インデックスが argmax になる**ことを確認した:

- 20 症例 × 12 点（先頭側 2・中央 8・末尾側 2）× 41 候補 = 9,840 通りすべてで argmax = 0
- NCC の分解能: `@0` 0.999 / `@±1` 0.991 / `@+5` 0.947 / `@+10` 0.914 / `@+20` 0.865
  → 1 枚のずれを識別できているので、0.999 は真の対応を意味する
