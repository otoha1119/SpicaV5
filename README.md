...
- **監視**: ターミナルは Stage 1 と同じ tqdm バー（画像枚数単位、末尾に loss と RMSE [HU]）だけ。TensorBoard は scalar（`loss/mse`、`train/rmse_HU`、`val/rmse_HU` と参照線 `val/rmse_input_HU` = 入力 − 教師、`time/*`、`train/lr`）と、epoch 末のフル 1024 パネル `images/full/fixed`（固定スライス = `log.full_slice`、**Stage 1 と同じ `PCD-002/PCD-002-215.png`**）/ `images/full/random`（epoch ごとに別のランダム `log.n_full_random` 枚、train ∪ val から。ラベルに名前と train/val）。パネルは **[EID-like1024 (input) | PCD1024 (teacher) | PCD-like1024 (output)]** の 3 列（`stage2/util/panel.py`。白余白・1px 枠・ラベル帯は Stage 1 と同じ。8bit、HU を `display_hu_min..max` = −1400..2100 で線形）。128 patch のグリッドは出さない。ディスクへの preview 出力は別フェーズ
...
- Stage 2 の設定の正は `stage2/configs/schema.py`。`configs/machines.yaml` は Stage 共通で、Stage 2 は使うキー（gpu_gen / host_data_root / container_data_root / num_threads / tb_port / pcd1024_dir / eidlike1024_dir / stage2_checkpoints_dir / stage2_tb_port）だけ検査する。Stage 1 の schema は未知キーを拒むので、Stage 2 のキーは `stage1/configs/schema.py` の MACHINE にも宣言してある（Stage 1 は使わない）
