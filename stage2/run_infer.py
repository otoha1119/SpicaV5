"""stage2/run_infer.py — Stage 2 推論（フォルダ丸ごと）の起動器（Docker 内で動く）。Stage 1 の run_infer.py と同じ流儀。[SpicaV5 新規 2026-09-10]
  ...
  6. パネル（save_panel）の相方 — input=eidlike なら 4・5 列目の実 EID（infer.yaml eid_slice、eid_dir からの相対パス）、input=eid なら 1〜3 列目の
     参照 PCD 症例（pcd_slice、eidlike1024_dir と pcd1024_dir からの相対パス）— は存在を確認する（空文字なら無し）
...
def main():
    ...
    try:
        ...
        if infer["save_panel"]:  # パネルの参照スライス（学習の固定パネルと同じ 5 列にするための相方）は存在を先に確認する
            if infer["input"] == "eidlike" and infer["eid_slice"] and not (Path(machine["eid_dir"]) / infer["eid_slice"]).is_file():
            ...
            if infer["input"] == "eid" and infer["pcd_slice"]:
                for root in (machine["eidlike1024_dir"], machine["pcd1024_dir"]):
                    if not (Path(root) / infer["pcd_slice"]).is_file():
                        raise ConfigError(f"infer.yaml の pcd_slice がありません: {Path(root) / infer['pcd_slice']}（eidlike1024_dir / pcd1024_dir からの相対パス。1〜3 列目が不要なら --pcd_slice \"\"）")
    ...
    argv = ["--weight_dir", str(weight_dir), "--input_dir", str(input_dir), "--out_dir", str(out_dir), "--device", device,
            "--index_cache_dir", str(Path(machine["stage2_checkpoints_dir"]) / ".case_index"), "--pcd1024_dir", str(machine["pcd1024_dir"]), "--eid_dir", str(machine["eid_dir"]), "--eidlike1024_dir", str(machine["eidlike1024_dir"]),
