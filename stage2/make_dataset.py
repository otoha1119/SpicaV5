...
値の規約（stored = HU + 1400）は変えない。PCD1024（別途変換）とはファイル名で対応する。学習時に補間しながら読む方式は取らない（ユーザー決定 2026-09-08）。
設計: docs/plans/20260908_stage2-unet-implementation-spec.md §3

  1. configs/dataset.yaml（scale / interp / input_size）と ../configs/machines.yaml（num_threads / container_data_root）を configs/schema.py で検証する（既定値なし）
  2. sh からの上書き（DATASET のフラグ、--input_dir / --output_dir）を実効値に反映する
  3. 入力を列挙する（Stage 1 の CaseIndex と同じ規則: 直下の症例フォルダを再帰的に、'.' 始まりは無視、PNG 以外は無視して枚数だけ表示）
  4. 出力先は container_data_root 配下で、OUTPUT_DIR と OUTPUT_DIR.tmp のどちらも存在しないこと（上書きしない。.tmp が残っていれば前回の中断なので手で消す）
  5. OUTPUT_DIR.tmp に 1 枚ずつ書く（num_threads 並列）。1 枚でも失敗すれば止めて .tmp を残す（黙って飛ばさない）
  6. manifest.yaml（由来・方式・症例ごとの枚数・実効設定）を書き、枚数を検算してから OUTPUT_DIR に rename する
...
使い方（通常は ../dataset_stage2.sh 経由）:
  python make_dataset.py --machine PC1 --config configs/dataset.yaml --machines ../configs/machines.yaml \
      --input_dir /workspace/stage1/checkpoints/<run>/infer/<weights>/PCD512_v2/<time>/full --output_dir /workspace/DataSet/EIDlike1024_v1 -- --interp bicubic
"""
...
PATH_FLAGS = ("--input_dir", "--output_dir")  # sh の変数を --flag で上書きできる（start2.sh の引数が最優先）
...
def split_overrides(tokens):
    """上書き列から sh 変数系のフラグ（--input_dir / --output_dir）を取り出し、残り（DATASET のフラグ）を返す。"""
    paths, rest, i = {}, [], 0
    while i < len(tokens):
        t = tokens[i]
        if t in PATH_FLAGS:
...
def main():
    ...
    p.add_argument("--machine", required=True, help="configs/machines.yaml のエントリ名（num_threads / container_data_root を使う）")
    ...
    p.add_argument("--output_dir", required=True, help="作成先（存在しないこと。container_data_root 配下）")
    p.add_argument("overrides", nargs=argparse.REMAINDER, help="'--' の後に上書き（DATASET のフラグ、--input_dir / --output_dir）")
    a = p.parse_args()
    ...
    try:
        ...
        applied = apply_overrides(rest, {"dataset": (dataset, DATASET)})
        check_dataset_values(dataset, machine)
        input_dir = Path(path_over.get("--input_dir", a.input_dir))
        output_dir = Path(path_over.get("--output_dir", a.output_dir))
        ...
        if os.path.commonpath([data_root, out_abs]) != data_root or out_abs == data_root:
            raise ConfigError(f"出力先は machines.yaml の container_data_root（{data_root}）配下のサブフォルダにしてください: {output_dir}")
        tmp_dir = output_dir.with_name(output_dir.name + ".tmp")
        if output_dir.exists():
            raise ConfigError(f"出力先が既に存在します（上書きしない。別の名前 / バージョンにする）: {output_dir}")
        if tmp_dir.exists():
