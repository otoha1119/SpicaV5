...
from util.run_paths import ckpt_path  # [SpicaV5]
...
class BaseModel(ABC):
    ...
    def ckpt_path(self, tag, kind):
        """[SpicaV5] checkpoint のパス規則（util/run_paths.py）: 重みディレクトリ <run>/latest/ | <run>/best/ | <run>/weights/epoch_NNN/ の中の kind。"""
        return ckpt_path(self.save_dir, tag, kind)
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if not self.isTrain or opt.continue_train:
                    ...
                    load_path = self.ckpt_path(load_suffix, f"net_{name}.pth")  # [SpicaV5]
    ...
    def save_networks(self, epoch):
        ...
        if not dist.is_initialized() or dist.get_rank() == 0:
            for name in self.model_names:
                if isinstance(name, str):
                    save_path = self.ckpt_path(epoch, f"net_{name}.pth")  # [SpicaV5] 重みディレクトリ（latest/ best/ weights/epoch_NNN/）の中へ
    ...
    def load_networks(self, epoch):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                load_path = self.ckpt_path(epoch, f"net_{name}.pth")  # [SpicaV5]
                net = getattr(self, "net" + name)
